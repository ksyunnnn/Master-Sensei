...
- **建玉なし**。**ライブ注文: SOXL 買い 50株・指値 $91.00・GTC（OrderId 5442384385、`trades` id=44）**。2026-09-22 18:39 JST に Saxo token が失効して以後 API 未確認。SOXL 2026-09-22 終値 $151.95 に対し指値は **-40.1%** で約定見込みなし。**必要資金 $4,550 が口座 $4,675 の 97.3% を予約現金として拘束する（K-031）ので、取り消すまで他の発注ができない**。取消しはユーザーが Saxo アプリで行う。
- **Claude に発注・取消しの機能は無い**（`src/saxo_client.py` の POST は OAuth の2箇所のみ、MCP は get 5つのみ）。**執行はユーザーが Saxo アプリで行う。PC も再認証も要らない**。Saxo 再認証が要るのは Claude が残高・建玉・注文を読んで照合する時だけ。ここを執行の前提条件として提示しない（2026-09-22 に約2時間そう提示して指摘を受けた）。
- **SOXL は 2026-09-22 終値 $151.95（+7.06%）で6連騰、2026-09-16 $103.97 から 5営業日で +46.1%**。2026-09-22 は 寄り $137.67 / 安値 $137.46（**寄り2分**）/ 高値 $152.64。higher low 連鎖 101.06→112.01→116.68→129.62→**137.46**。断面は 2営業日連続で SOXX > QQQ > IWM > SPY の単調＝半導体固有。
- **押し目指値買いにエッジは検出できていない（K-087）**。指値のどの水準でも中央値＝損切り水準、勝率 17〜39%。ただし**「エッジが無い」と「入るべきでない」は別**で、risk 4% を守った1発は ADR-028 上は正当。2026-09-22 は押し目を提示しながら見送り側に寄せ、寄り2分の安値（結果的に当日の底）を取れていない。#46 の指摘と同型なので、**エントリー可否を問われたらルール内の入り方を必ず作り、統計と並べて出す**。
- **ADR-028 の「SL 引き上げは higher low 確定後」は確定条件が未定義**。緩い定義（前営業日の安値超え）だと 20営業日保有の平均が固定損切りの約 1/7 になる（K-088）。運用は厳密なピボット定義に限定する。
- **regime は 2026-09-22 付 risk_on 1.21 を記録済み**（2026-09-21 の 1.29 から USD weak→normal の1点のみ変化、中身は FRED の観測日進行）。
- **データ鮮度（2026-09-23 05:40 JST 時点）**: マクロは 9/22、日足・5分足は **9/21 まで。2026-09-22 分が未取り込み**なので判断前に `update_data.py` を回す。
- **Tiingo が約100 req/時以下で HTTP 429**（2026-09-14 23:44 JST）。`docs/api/tiingo/rate-limits.md` の Power プラン記載と矛盾、未確認。
- **価格 parquet の品質問題（#38、未修正）**: SOXX 日足 OHLC が 2024-03-07 分割で未調整、5分足に米国休場日の合成バー。2026-09-21 の 5分足は寄り 25分が前日終値の平坦バーだった。集計前に除外する。
- **カタリスト**: 9/24 米中首脳会談（議題に trade・tariffs・Taiwan・AI）→ 9/30 21:30 JST 8月 PCE → 10/1 05:30 JST Micron Q4 FY26 決算。
- **jev（TypeSafe の判断モデル）を判断エンジンに据える検討を継続（#47）**。**課金が発生する API 呼び出しは回数と目的を事前に提示して承認を得る**（2026-09-18 に無断実行して指摘を受けた）。
- **参照**: K-023/K-031/K-041/K-055/K-056/K-072/K-074/K-085/K-086/K-087/K-088/K-089、ADR-028/ADR-031/ADR-035/ADR-039。実施記録は #49、開いている作業は issue #27/#31/#34/#37/#38/#39/#40/#42/#47/#49。
