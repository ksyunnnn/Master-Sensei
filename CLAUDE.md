...
- **Saxo token 2026-07-01 19:51 JST 再認証済（oauth_init）**。`/sync-saxo` 3層照合 break ゼロ（ライブ建玉0＝フラット、台帳44行、trades一致）。**keepalive は今回 ~5時間稼働**（token 4回ロール: 21:28/22:23/23:18 JST）→ 2026-07-02 00:42 JST に refresh 失効で `needs_reauth` クリーン終了。**前スタンスの「45-55分で kill」より大幅改善**＝環境依存で不安定なだけで keepalive 自体は機能する（ADR-025 の非保証性は維持）。**現在 token は失効中**、次に Saxo 要時は oauth_init 起動。margin 拡大（300→600s）未決。
- **6/30 の入金 ¥100,000 は「外部銀行入金」でなく口座間振替（CASHINTRTP）**だった（`/sync-saxo` で EOD bookings 記帳→台帳反映、`deposit` 写像を確認・5件目の観測）。`docs/api/saxo/booking-fields.md` の「外部銀行 deposit の符号/symbol 表現 未確認(70%)」は**未解消のまま維持**（本口座にまだ真の外部入金履歴が無い）。
- **2026-07-01 米セッション: SOXL 半導体固有キャピチュレーション**。寄り $237.78（−10.8%ギャップ）→ 09:35 安値 $230.10 → V反発 $246.39（09:50）→ 失速し **session 安値 $224.04（10:55 ET）**→ 11:52 ET **$227.21（−14.8%）**。**SPY +0.24% 緑・VIX 16.7 平穏の中での完全デカップリング**＝市場全体でなく半導体固有。**discrete な Section 232「決定」は無し**（7/1 は大統領への内部報告期限で公開イベントでない）＝無触媒の技術/ポジション売り。6/30 スタンスの「押し目 $249-250」は弱シェルフで**無効化・貫通済**。**口座フラットのまま dip-buy 未エントリー**（reversal 未確定・NFP 前で「確認待ち/待機」を提示、成行追随・224割れ買い増しは禁止）。実効 floor $224、割れれば 211→192。regime は risk_on 0.79 で 6/30 と同一（DB 記録スキップ）。**この価格は 11:52 ET 時点の途中経過**（引け未確定、次セッションで update_data 後に再取得）。dip-buy タイミングは K-049（下落日は寄り後30分が底でないことが多い）を適用。
- **次アクション: 2026-07-02 21:30 JST NFP（第二バイナリ）と SOXL 反転確認**。日次ワークフロー（update_data → /scan-market → /update-regime → /sync-saxo）。トラッカーは #11（KPI設計、保留）のみ。dip-buy 判断は市場が動くので都度再評価。
