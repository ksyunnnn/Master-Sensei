...
- **Saxo token は失効中（2026-09-01 02:52:38 に expire）。ブラウザ再認証が要る**。`/sync-saxo` は exit 2 になる。**「対処済み」と報告しない（K-077）**。
...
- **8月分の入金 ¥200,000 が台帳未計上のまま**（`reports/bookings` に現金行が1本も来ない。最終 deposit は 7/31）。手書きしない（issue #35）。`sync_saxo.py` の「✓ 差分なし」は建玉と注文の話で現金の整合を意味しない。
