...
- **建玉あり: SOXL 25株 @ $119.879(8/20 約定、`trades` id=43 / OrderId 5435911635)。取得原価 $3,004.84 / ¥477,978、投入割合 89.5%(cap ~90%)**。8/24 プレ($115.01, 05:56 ET)で -$129.59(取得原価比 -4.31%、口座比 約 -3.8%)。**この建玉に保護逆指値が入っているか未確認** — `trades` に id=43 対応の OCO 脚が無く、Saxo token 失効(`get_positions` が AUTH_REQUIRED)で `get_open_orders` も引けていない。**次セッションの最初の作業は再認証 → 保護注文の確認 → `/sync-saxo`**。先週の24株は保護逆指値が $120.03 で機能した(K-071)。
- **8/21〜8/24 の下げは半導体固有で、震源はサムスン**。8/21 引けの断面は IWM +0.769% > SPY +0.409% > QQQ +0.353% > **SOXX -0.440%** と半導体だけが単独安、8/24 プレも同じ序列(SOXX -1.53% / QQQ -0.55% / SPY -0.14% / IWM -0.05%)。サムスンは 8/21 に ₩90-110兆($65-80B、従来記録の約5倍)の株主還元を承認したが**配当/自社株買い/消却の配分を 2027年1月へ先送り**し、8/24 に -8.35%。一方 **SK hynix は +2.4%、NVDA はプレで +0.19% と無風**で、AI 需要そのものへの疑いではなくメモリ銘柄への横並び売り。原油はむしろ緩んだ(Brent -1.66%)ので、同日の Bessent 対イラン制裁発表を driver に据えるのは salience bias。
- **継続するかは未決着。ただし「金利軸が生きているから反発が減衰する」という 8/24 前半の私の説明は撤回する**。[[K-074]]の実測で、8/17-19 の暴落は金利で説明できないと判明した — **8/17 は30年金利が +6bp の 5.31%(19年ぶり高値)まで上げた日に SOXX が +1.58%(SPY -0.47% に対し逆行高)、暴落した 8/18 は金利 -3bp、8/19 は -9bp** で、実質金利も MOVE も同方向に下げている。非継続側の [[K-068]](2日連続で QQQ を下回った翌日も下回る確率 42.0%/n=283 < 無条件 48.4%)と [[K-039]](プレ下落日の当日下落は 39-47%)は生きており、[[K-047]]の「金利軸併走で減衰」を今回に適用する根拠は消えた。[[K-064]](n=466)より **KOSPI の値動きは SOXL の寄り後と無相関**。判断は 22:30 JST の寄り後、実出来高のついた価格が出てから。
- **金利の水準そのものは判断に使わない([[K-074]])**。名目30年は SOXX に対し5年 n=1289 で r=-0.035 / t=-1.24 と有意でない。実質金利(-0.573%/10bp, t=-3.30)と期待インフレ(+0.928%/10bp, t=+3.18)が**逆符号で打ち消し合う**ため。水準・トレンド除去後の相対位置・5日変化のどれでも翌日リターンを分ける帯が無かった。参考値として30年名目は 8/17 に 5.31% が2026年最高、8/21 5.276%、2026-02-27 の 4.63% が底。
- **issue#26 は前半のみ完了**: US30Y(FRED DGS30)に加え **REAL10Y/REAL30Y(実質金利, DFII10/DFII30)・BREAKEVEN10Y(期待インフレ, T10YIE)・MOVE(債券ボラ, ^MOVE)** を収集開始(各5年 backfill 済)。**後半(レジーム判定への組み込み)は「実装しない」を提案中で、ユーザー判断待ち** — データが水準閾値を支持しないため。これらは `update_data.py` のサマリーで「帰属専用・レジーム判定には使わない」ブロックに分けて表示している。**MOVE は [[K-073]] の基準では未検証**(一次公表元 ICE BofA に無償の照合先が無い)。
- **今日の記録に誤りが残っている(未処理)**: 8/22 05:00 JST 登録の「米30年債 5.273%」を **impact=negative** としたが、根拠にした「金利が半導体を潰した」という帰属が上記で崩れた。**impact の事後修正は [[K-056]](realized price 混入)の危険があるため `/review-events` で処理する**。issue#26 のコメントと GDR/ADR には未反映。
- **指標の用途は4段([[K-075]])**: ①方向 ②値幅(sizing) ③帰属 ④異常検知。**予測の検定(①②)で落ちても③④では生きるので「判断は何も変わらない」と結論しない**(8/24 に私が金利についてそう言い切った誤りの是正)。金利変数は①②で棄却・③④で有効。**翌日の値幅推定は SOXX 自身の直近5日ボラが最良**(t=+3.92、金利系は全て t≈1.1〜1.4)で、sizing に金利変数は不要。
- **参照**: K-039/K-041/K-047/K-055/K-063/K-064/K-068/K-070/K-071/K-073/K-074/K-075。**今週のカタリストは 8/27 05:20 JST NVIDIA 決算 → 8/28 05:45 JST Marvell 決算 → 8/28 23:00 JST Warsh 議長のジャクソンホール基調講演(就任後初)**。9/16-17 FOMC は据置予想が約65%。財務省の長期債バイバック倍増は 9/9 発効〜11/4。
