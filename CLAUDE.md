...
- **建玉なし**。**ライブ注文: SOXL 買い 50株・指値 $91.00・GTC（OrderId 5442384385、`trades` id=44、2026-09-21 23:25 JST 時点で `Working`）**。SOXL 現値から大きく下で約定見込みなし、ユーザーは棄却前提と表明済み。**必要資金 $4,550 が口座 $4,675 の 97.3% を占め、市場オープン中は予約現金として差し引かれる（K-031）ため、取り消すまで他の発注ができない**。
- **SOXL は 2026-09-16 終値 $103.97 → 09-21 終値 $141.93（4営業日 +36.5%）**。9/21 は 寄り $132.07 / 安値 $129.62 / 高値 $144.00。higher low 連鎖 101.06→112.01→116.68→129.62 が成立しており、**SL は $129 台の下へ引ける**（ADR-028）。4営業日とも押し目らしい押し目が無い。
- **このセッションは分析だけで終わり、+36.5% の上昇にポジションを取っていない**（#48）。#46 の指摘と同型なので、エントリー可否を問われたら**ルール内の入り方を最低1案は必ず作り、見送り案と並べて出す**。
- **regime は 2026-09-17 付 risk_on 0.64 のまま。2026-09-21 の判定（risk_on 1.29、VIX normal→low / VIX_TERM contango→steep_contango / OIL crisis→high）は未記録**（#48 の開いている事項）。
- **K-029 の T+5（5日平均 -3.87%）が現在データで再現しない**（再計算 n=38 で +0.19%、符号が逆。T+1 は再現）。扱い未決、#48 参照。**知見を引く時は confidence/verification_status ラベルを鵜呑みにせず、その場で再計算する**。
- **Saxo token は 2026-09-21 21:56 JST に再認証済み**。keepalive はセッション終了で停止するので、次セッションで失効していれば `scripts/saxo_oauth_init.py` を Claude が起動する。
...
- **データ鮮度（2026-09-22 16:54 JST 時点）**: マクロは 9/21、日足・5分足は 9/18 まで。**9/21 の日足・5分足を取り込んでいない**ので判断前に `update_data.py` を回す。
- **カタリスト**: 9/24 米中首脳会談（議題に trade・tariffs・Taiwan・AI）→ 9/30 21:30 JST 8月 PCE → 10/1 05:30 JST Micron Q4 FY26 決算。
- **jev（TypeSafe の判断モデル）を判断エンジンに据える検討を開始（#47）**。Cloudflare Workers AI 経由で疎通済み。**課金が発生する API 呼び出しは回数と目的を事前に提示して承認を得る**（2026-09-18 に無断実行して指摘を受けた）。
- **参照**: K-029/K-031/K-041/K-055/K-082/K-085、ADR-027/ADR-028。実施記録は #48、開いている作業は issue #27/#31/#37/#38/#39/#40/#42/#47/#48。
