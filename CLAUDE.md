...
- **建玉なし**。**ライブ注文: SOXL 買い 50株・指値 $91.00・GTC（OrderId 5442384385）、保護脚なし、`trades` id=44（placed）**。口座 約 $4,715 の約96.5%で ADR-028 の cap 超。2026-09-17 のプレ $110.00 から -17.3% 下にあり、約定していない。Saxo token 失効のため Working かは未確認。
- **エントリー可否を問われたら、ルール内の入り方（無効化ライン・損切り距離・risk 4% からの株数）を最低1案は作り、見送り案と並べて出す**。2026-09-16 は $91 指値の扱い4案だけを出し、現値付近で入る案を出さなかった（#46）。
- **Saxo token は 2026-09-16 09:45 JST に失効したまま**。再認証は `scripts/saxo_oauth_init.py`（ユーザーはブラウザログインのみ）。
...
- **市況は stale**。最後の観測は 2026-09-17 18:07 JST（SOXL プレ $110.00＝9/16 終値 $103.97 比 +5.8%、SOXX +1.86%、QQQ +1.13%、FOMC 25bp 利上げで 3.75〜4.00%）。regime も 2026-09-17 付 risk_on 0.64 のまま。**2026-09-21 20:40 JST 時点で 9/18 以降を一度も見ていない**ので、判断前に取り直す。
- **データ鮮度（2026-09-21 20:40 JST 時点）**: マクロは 9/17 まで、日足・5分足は 9/16 米引けまで。フィラデルフィア連銀 9月調査（events id 475）と 9/18 昼の日銀の結果は未確認のまま。
- **カタリスト**: 9/24 米中首脳会談 → 10/1 早朝 Micron 決算。
...
- **参照**: K-041/K-055/K-072/K-074/K-083、ADR-027/ADR-028。実施記録は #46、開いている作業は issue #27/#31/#37/#38/#39/#40/#42/#47。
