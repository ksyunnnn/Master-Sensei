...
- **建玉あり: SOXL 25株（`trades` id=43 / OrderId 5435911635）。取得原価 $3,004.84 / 建値 $120.194**（Saxo 側 open_price $119.879 は手数料抜き）。2026-08-28 終値 $111.34 基準で含み **-$221（建値比 -7.37%）**。**保護逆指値なし**。出口設計は issue #31（4択は未決のまま3セッション経過）。
- **無効化ラインは引き直し済み**: $112.00 は 8/28 に割れた（高値 $121.20 →安値 $110.21、終値 $111.34 の大陰線・出来高6,100万）。次のスイング安値は **8/24 の $106.00**。
- **口座（2026-08-31 18:50 JST、77800/T126816・JPY）**: NAV ¥699,370.53 / 現金 ¥255,850。**8月分の入金 ¥200,000 が台帳未計上**（Saxo `reports/bookings` に未着。`--full` でも cash 8行のまま、最終 deposit は 7/31）。**手書きしない。翌営業日以降の `/sync-saxo` で入るのを待つ**（issue #35）。
- **`sync_saxo.py` の「✓ 差分なし」は建玉と注文の話で、現金の整合を意味しない**（3層とも buy/sell しか見ていない）。NAV は sizing の分母なので、現金の取り込み漏れは分母を過小評価させる（issue #35）。
- **4択を出し直す時は「いつまでに決めないと、どの選択肢が既定で選ばれたことになるか」を先に置く**。2026-08-27 はこれを明示せず、決定が出ないまま期限を通過させた（issue #32）。
- **執行経路を「対処済み」と報告しない（K-077）**。keepalive は3回続けて既知のカタリスト時刻をまたげずに失効した（8/26 79分 / 8/27未明 / 8/27夜 74分）。**3回目の原因は未特定**（issue #22）。カタリストをまたぐ経路は**使う直前に実際に叩いて確認する**。
- **`saxo_oauth_init.py` は待機中ずっと DB の排他ロックを保持する（issue #34、未修正）**。`--wait-min` を長く取ると他の全プロセスの DB アクセスを止める。終わったら必ず落とす。
- **`master-sensei-live` MCP サーバは起動時のコードを保持する**。`src/` を直しても再起動するまで旧挙動のまま。2026-08-31 の ADR-040（基準足の鮮度検査）も未反映のはずなので、次セッションで MCP と in-process の出力が食い違ったらこれを疑う。
- **regime のスナップショットは全6列で観測日が保存されていない**（issue #14）。8/26 分は観測日を `reasoning` に文字列で埋めて手動記録した。8/25 以前は書き換えていない。
- **カタリスト**: 8/28 Marvell 決算・Warsh ジャクソンホール基調講演は通過済み（結果未検証、`/review-events` 対象）→ **9/7 レイバーデー（米国休場）** → 9/8 カナダ報復関税発効（電子機器を含む）→ 9/17 03:00 JST FOMC。
- **参照**: K-041/K-055/K-059/K-060/K-063/K-064/K-065/K-070/K-074/K-076/**K-077**、ADR-040。
