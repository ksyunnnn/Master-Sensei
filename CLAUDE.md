...
- **8/4→8/5 で半導体は往復した**: SOXL は 8/3 $116.71 → 8/4 **$139.90(+19.87%)** → 8/5 **$132.07(-5.60%)**。8/5 は 寄り$135.54→**高値$144.34**→終値$132.07 と場中で往復。8/4 の上げのドライバーは特定済み＝**中東デエスカレーション→原油下落(Brent -5.6%)→AIトレード回復**の広範リスクオン(S&P +1.79% で最高値、テック +4.20%)で、**半導体固有の材料ではない**。8/5 の下げは **AMD 決算(8/5 05:05 JST)** 主導＝全項目ビート＋Q3ガイド超過だが **capex $808M(見積り$298Mの2.7倍)** で return/資金の持続性が問われた側(K-053/K-054)。
- **口座はフラット**(建玉0・注文0)、¥594,744(= $3,768 @157.84)。**円は1週間で 163.30→157.84 と3.3%の円高**でUSD購買力が目減りしている。**Saxo token は再び失効中**(8/5 21:47 に keepalive が停止)。次セッションで Saxo に触るなら**まず `scripts/saxo_oauth_init.py` の再認証**。今回の停止は**スリープ由来ではなく roll 成功3回の後に落ちた**別の失敗モードで、ログ付きで **issue#22** に追記済み。
- **8/5 の押し目エントリーは分析のみで未発注**。設計(指値$134.90/部分利確$141.50×3株/TP$151.09/SL$115.50/7株・投入25%/BE 0.742%)を提示したがユーザー選択前に終了。**trades にも未記録で正しい**(建玉0)。結果的に指値も部分利確も約定水準に触れていたが、発注していないので実績ではない。
- **regime は 8/5 で記録済み**: `risk_on` (+1.21)。**信用が normal→tight に変化**(HY 2.84→2.78)。VIX 15.81 / VIX3M 18.95 / Brent **$79.65**(7/31 $90.12 から -11.6%) / YC 0.45 / VXN 25.48(8/4) / USD_INDEX 119.70(7/31・stale)。**VXN-VIX スプレッドは 97%ile 圏**でテック固有ストレスは継続。
- **今セッションの記録**: **K-067**(半導体の大陽線は SMA20 の上下で先行リターンの符号が分かれる。4閾値すべてで T+5 中央が 下<上。`assess_flow` は 1d/3d 主体なので**下落局面の戻しを bullish と誤読**する — 8/4 に bullish+0.80 → 8/5 は -5.60%。`sigma_position` を必ず併読。なお「深い指値ほど逆選択で不利」は n=9 でのみ見え全期間 n=1262 で再現せず**棄却**)。**issue#25**(assess_flow の文脈欠落。ADR が要る) を起票。**予測 #13 は false で解決**(SOXL 8/5終値 $132.07 ≥ $102.00、Brier 寄与 0.384、平均 0.343→**0.346**)。ただし当該ラダー($102/$98/$94)は 8/4安値$129.66・8/5安値$130.00 で**一度も約定圏に入っておらずデプロイ0**＝K-041 の2例目。
- **`/sync-saxo` の「✓差分なし」を信用しない**は継続(**issue#20**、同日往復は純数量照合では構造的に不可視)。8/5 の sync は3層一致だった。
- **参照**: K-040/K-041/K-044/K-047/K-052/K-054/K-055/K-057/K-058/K-059/K-063/K-064/K-065/K-066/K-067。**issue#24 は T+5=8/10 確定後に K-063 の機構を引き直す**(8/5 は SPY も -0.20% で引けたため悪い象限は終値では不成立。プレでは SPY +0.24% に見えていた＝K-064 の実例が1つ増えた)。scan-market に通貨・FXカテゴリが無い → **issue#21**。
