...
- **現在フラット(建玉ゼロ・未約定注文ゼロ)**。現金 **¥593,528**(全額 spending_power、口座 77800/T126816)。7/28-30 の押し目ラダー一巡が **7/30 に12株を$112.911で一括決済**(OrderId 5429337890)して完結: 建値平均$100.9617(4@$107.00＋4@$100.885＋4@$95.00)→ 実現 **+$135.34(+11.10%)/+¥16,643(+8.34%)**。stopには一度も触れず自分の判断で利確＝K-059(壊れたstopはエグジットの決定権を奪う)の直後に、保護脚が機能する範囲内でエグジットを選べた形。
- **⚠ DBが実態とずれている(2件、次セッション最優先)**: ①**トラッカー#18** — `trades` 判断層に上記7/30決済が未反映(id=28/29/30 が filled のまま、id=31〜36 のOCO脚が placed のまま)。`/sync-saxo` で SenseiDB メソッド経由の修正が必要。②**トラッカー#19** — Saxo残高と台帳が**ちょうど¥100,000乖離**。未計上の入金¥100,000があると推定(確信度90%、入金¥650,000と置くと数字が完全に閉じる)。**本人確認が要る**。確定すれば通算は入金¥650,000→¥593,528＝トレード通算 **-¥56,472(-8.7%)**。
