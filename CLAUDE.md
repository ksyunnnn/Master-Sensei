...
- **8/3 深夜に4株のスキャルプが入っている(未記録)**。23:00:05 JST 建て $107.769 → 23:22:33 決済 $110.82、22分半で **net +$9.99(+¥1,217.58)**。PositionId 建て7695071393/決済7695191689。**`trades`(判断層)に未記録＝エントリー理由の本人申告待ち**。`account_transactions` は reports 反映待ちで、次回 `/sync-saxo` で自動的に入る(手書きしない)。現金は settled ¥593,528＋未計上¥1,217.58。
- **`/sync-saxo` の「✓差分なし」を信用しない**。上記スキャルプの直後に2回実行して2回とも差分なしと誤報した。同日建て同日決済は3層(建玉/台帳/注文)すべてで不可視になる。`closedpositions` か残高の `transactions_not_booked`/`net_positions_count` を見ること。**issue#20** に起票済み。
- **8/3は歴史的な往復だった**: SOXL 寄り$104.09→安値$102.09(-10.25%)→**+1.43%までプラス転換**(安値から+14.0%)。SOXX -3.46%→+0.48%。朝は SPY+0.60%/SOXX-3.09% で **SOXX-SPY差 -3.69pp＝下位2.3%ile**(「SPYプラス&SOXX≦-2.5%」は1284日中5回=0.4%)だったが、寄付90分で乖離が消えた。**予測 id=11(SOXX終値>504.89, conf 0.18)は閾値付近で往復中、id=13(見送りが正解, conf 0.62)は外れる方向**。
- **今日の主な学び(すべて記録済み)**: **K-063**(押し目可否の判別子は VXN/VIX でなく**指数間の乖離**。SOXX単独急落≦-3%はn=107でT+5+1.99%とエッジあり、SPY>0を足すと符号反転し乖離が深いほど単調悪化。K-048は結論◯・機構×) / **K-064**(寄付前の情報は寄り後を予測しない。ギャップn=1284・KOSPIn=466・寄り30分n=161の3系統すべて無相関。K-060をn=1から一般化) / **K-065**(寄りでギャップダウンを買い同日決済はコスト後**マイナス期待値**。K-061の「未確認」は実は負)。**K-060/K-061 は validated に昇格、K-048 は tested に更新**。
- **regime**: 8/3記録 **risk_on(1.00)**。ただし前回7/31(0.64)との比較は vintage 差のアーティファクトで、7/31確定値なら1.14＝実質微低下。実体変化は Brent $90.12→$83.86(-6.9%)による OIL の high→elevated のみ。VXN-VIXは7/31で**10.01＝98%ile**が5営業日continue。
- **参照**: K-040/K-041/K-044/K-047/K-052/K-054/K-055/K-057/K-058/K-059。**Saxo keepalive はセッション中に無告知で死ぬ**(8/3に3回)。Claude Code 公式では **Desktop scheduled tasks(`.claude/scheduled-tasks.json`、最小1分)** がセッション外実行の正規手段で、ADR-025 が禁じた launchd/disown とは別物 → **issue#22**。scan-market に通貨・FXカテゴリが無く日米協調円買い介入を取りこぼした → **issue#21**。
