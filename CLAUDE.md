...
- **8/4 は半導体の歴史的リターン**: SOXL $116.71→**$139.90(+19.87%)** / SOXX **+6.80%** / QQQ +3.40% / SPY +1.80%。**SOXX-SPY差 +5.00pp** は前日8/3の **-3.69pp**(下位2.3%ile)のほぼ鏡像。SOXL は 8/3安値$102.09 から **+37.0%**。**この上げのドライバーは未特定**(`/scan-market` 未実行、1回の検索では8/4固有の材料を特定できず)。AMD決算は 8/4引け後=8/5 05:05 JST で**この上げの原因ではない**。
- **口座はフラット**(建玉0・注文0)。¥594,744(settled ¥593,526.41＋未計上¥1,217.59)。**Saxo token は失効中** — セッション中に一度再認証したが keepalive がマシンスリープ中に期限を追い越して死亡(機構はログ付きで **issue#22** に記録)。次セッションで Saxo に触るなら**まず `scripts/saxo_oauth_init.py` の再認証が要る**。
- **予測 id=13 が未resolve(deadline 8/6)**。「8/5 の SOXL終値 < $102.00」= 8/3見送りが正解、conf **0.62**。8/4終値 $139.90 で **false がほぼ確定**だが判定日は8/5終値なので、**8/6朝(8/5終値のparquet反映後)に resolve する**。conf 0.62 なので Brier への寄与が重い。
- **regime は 8/4 データで未判定**(`/update-regime` 未実行)。8/4値: VIX 16.50 / VIX3M 19.34 / **Brent $79.14**(8/3 $83.86 から **-5.6%**、7/31 $90.12 からは -12.2%) / YC 0.43 / HY 2.78(8/3) / US10Y 4.70(8/3) / VXN 24.77(8/3) / USD_INDEX 119.70(7/31・stale)。**Brent の続落が唯一の実体変化**候補。
- **今セッションの記録**: **K-066**(円口座の米国株往復コストは USD建て表示P/Lに現れない**FX変換分**を含む。8/3往復は gross$12.204→手数料-$2.21→FX変換-$2.24→実現**+¥1,216.44(≒+$7.74)**。前回記録の「+$9.99」は FX変換前で **29%過大**だった。往復all-inは通常0.34〜0.65%、小notionalでは1%超)。**issue#23**(8/3スキャルプの trades 未記録・エントリー/エグジット根拠の本人申告待ち) / **issue#24**(K-063の最大反例。T+5=**8/10**確定後に機構を引き直す) を起票。
- **`/sync-saxo` の「✓差分なし」を信用しない**は継続。台帳にデータが揃った後も同日往復は**純数量照合では構造的に不可視**と確認済み(**issue#20** に切り分け記録)。存在ベース照合(台帳の order_id が trades.broker_ref に無い)が4本目の軸として必要。
- **参照**: K-040/K-041/K-044/K-047/K-052/K-054/K-055/K-057/K-058/K-059/K-063/K-064/K-065/K-066。scan-market に通貨・FXカテゴリが無い → **issue#21**。
