...
- **前サイクル決着(7/23)**: SOXL 13株(10@$196＋3@$165.40)を**$160.831で一括手仕舞い**(OrderId 5427302702)。実現**-$383.11(-15.5%)/-58,417 JPY**。trades id=24/25 close済、OCO脚 id=26/27 cancelled。**エグジット理由は本人申告で判明「SLを曖昧な状態にしていたため損切り」**＝テーゼ崩壊でなく損失上限を定義できない状態の解消。stop$100は一度も接触せず(最安7/17 $116.47)、決済価格$160.831はチャート上の何にも対応していない→**壊れたstopは損失額でなくエグジットの意思決定権を奪う(K-059)**。discipline_score=3(損切り実行は正。欠陥は口座36%相当のstopを保護と見なし9日是正しなかった前段)。通算: 入金¥550,000→現在¥476,885(**-13.3%**)、決済21件・勝率47.6%。
...
- **参照**: 新規knowledge **K-057**(ラダー中間段の等間隔配置はレベルの谷間に落ちる/1段目は押し幅・2段目以降はレベル)・**K-058**(セクター急落日にVIX低下ならセクター固有と判定してよい。逆は不成立)・**K-059**(機能しないstopは損失額でなくエグジットの意思決定権を奪う。判定は「到達時の損失を口座比%で言えて受け入れられるか」。言えなければstopでなくサイズを落とす)。既存K-052/K-054/K-055。予測#10(SOXLが7/31までに終値$128.15超え、conf 0.35)未解決。Saxo関連注文の**距離制限実測**(+43.2%通過/+52.6%拒否、安全域は+40%以内)は`docs/api/saxo/order-fields.md`に記録済。トラッカー新規#16(get_open_ordersがRelatedOpenOrdersを落とす=無保護と誤読しうる)/#17(sync_saxoが約定翌朝に全年mirror空振り＋close_trade誤提案)。SOXS禁止は既確定。
