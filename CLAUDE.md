...
- **建玉なし**。**ライブ注文: SOXL 買い 50株・指値 $91.00・GTC（OrderId 5442384385、`trades` id=44）が 2026-09-24 21:47 JST 時点で `Working`**。SOXL 2026-09-24 終値 $146.33 に対し指値は -37.8% で約定見込みなし。**必要資金 $4,550 が口座（`spending_power` ¥729,968 ≈ $4,599）の大半を予約現金として拘束する（K-031）ので、取り消すまで他の発注ができない**。取消しはユーザーが Saxo アプリで行う。
- **Claude に発注・取消しの機能は無い**（`src/saxo_client.py` の POST は OAuth の2箇所のみ、MCP は get 5つのみ）。**執行はユーザーが Saxo アプリで行う**。Saxo 再認証が要るのは Claude が残高・建玉・注文を読んで照合する時だけで、執行の前提条件として提示しない。
- **コマンドの依頼範囲で止める**。2026-09-24 に `/sync-saxo` が「差分なし」で終わった後、依頼外の注文・残高の解説を足して「指示以外のことはするな」と指摘された。ただし注文と残高の状態は把握しておく（同日のユーザー指示）。
- **SOXL は 2026-09-24 終値 $146.33（+0.05%）**。プレ市場で -6.4%（金利ショック: 10年債 5.1%台、30年債 5.446%）→ 寄り $138.47 → **寄り1分（09:31 ET）で安値 $136.90** → 引け前日比ほぼ横ばい。2026-09-22 に続き、寄り直後の安値がその日の底だった。**higher low 連鎖（…129.62→137.46→139.58）は 2026-09-24 の $136.90 で途切れた**。
- **押し目指値買いにエッジは検出できていない（K-087）**が、「エッジが無い」と「入るべきでない」は別。**エントリー可否を問われたらルール内の入り方を必ず作り、統計と並べて出す**（#46・#49 の指摘）。2026-09-24 はユーザー方針「下げが続けば押し目指値」に対し A/B/C 案を 22:31 JST に提示したが、条件（安値を5分連続で割る）は成立しなかった。
- **ADR-028 の「SL 引き上げは higher low 確定後」は確定条件が未定義**（K-088）。運用は厳密なピボット定義に限定する。
- **寄り後の監視運用（2026-09-24 に実施）**: `Monitor` + 調査サブエージェント（`SendMessage` で追加ラウンド）+ `PushNotification`。節目の抜けは寄り直後に往復するので5分連続で確定させる。`Monitor` は30分で失効し、再起動すると ±2% の基準値がリセットされて変動を取りこぼす（2026-09-24 に -3.5% の下げが通知されなかった）。60秒ポーリングは1分未満の安値を取りこぼす。
- **regime は 2026-09-22 付 risk_on 1.21 が最新**。マクロは 2026-09-24 まで取り込み済みだが未判定。
- **データ鮮度（2026-09-25 05:20 JST 時点）**: マクロは 9/24、日足・5分足は **9/23 まで。2026-09-24 分が未取り込み**なので判断前に `update_data.py` を回す。
- **Tiingo が約100 req/時以下で HTTP 429**（2026-09-14 23:44 JST）。`docs/api/tiingo/rate-limits.md` の Power プラン記載と矛盾、未確認。
- **価格 parquet の品質問題（#38、未修正）**: SOXX 日足 OHLC が 2024-03-07 分割で未調整、5分足に米国休場日の合成バー。集計前に除外する。
- **カタリスト**: 米中首脳会談は 2026-09-24 に半導体の成果なしで終了（Board of Trade のみ）。State Dinner（9/25 07:45 JST、Huang・Su 出席）と 9/25 の成果文書の有無 → 9/30 21:30 JST 8月 PCE → 10/1 05:30 JST Micron Q4 FY26 決算。10月 FOMC の利上げ確率は 70% 台（2026-09-24 報道）。
- **jev（TypeSafe の判断モデル）を判断エンジンに据える検討を継続（#47）**。**課金が発生する API 呼び出しは回数と目的を事前に提示して承認を得る**（2026-09-18 に無断実行して指摘を受けた）。
- **参照**: K-031/K-041/K-055/K-056/K-074/K-087/K-088、ADR-028/ADR-031/ADR-034/ADR-035/ADR-039。実施記録は #50、開いている作業は issue #27/#31/#34/#37/#38/#39/#40/#42/#47/#50。
...
- **自分の出力を否定されたら、ルール・文言・報告形式を書き換えて応答しない。指摘の原因になっている実体の欠陥（コード・データ・記録の誤り）を先に直す**。ルールに触ってよいのは実体を直した後か、実体がそもそもルールしかない時だけ（2026-09-01、`/sync-saxo` の報告を怒られて報告ルールを書き換え「問題のすり替え」と拒否された。実体は現金の照合層が無いコードの欠陥で、`reconcile_cash()` 実装→issue クローズ→ルール修正が正しい順序だった）
- **ユーザーが特定の対象について話している時、backlog 全体や隣接 issue へ勝手に広げない**。広げる価値があると判断しても実行に移さず、1行で提案して元のスコープを閉じる（同日、`/sync-saxo` の話から open issue 22件の棚卸しを始めて「勝手に問題を拡大するな。時間無駄にするな」と止められた）
...
### 形式と機構（公式 Auto memory、https://code.claude.com/docs/en/memory.md）

**この5点を守らないと、書いても運用に乗らない。**

1. **セッション開始時に読み込まれるのは `MEMORY.md` の先頭200行 / 25,000バイトまで**。
   超過分は**黙って捨てられる**（末尾のエントリから不可視になる）。
   行動を変えたいことは必ず索引の1行に書く。
2. **個別ファイルの本文は、Claude が明示的に開くまで読まれない**。
   索引の1行だけで行動が変わる粒度に要約する。本文の括弧書きに埋めた規範は届かない。
3. `type` は **`user` / `feedback` / `project` / `reference` の4種のみ**（公式定義）。
4. **`modified` / `originSessionId` / `node_type` は Claude Code が書く欄。手で書かない・消さない**。
   `modified` は書き込み時に自動で刻まれる（v2.1.214+）ので手書きは捏造になる。
   `originSessionId` を消すと次の書き込みで別 ID が刻まれ、真の出自が永久に失われる。
   **これらが自動で刻まれるのは Write/Edit ツールで書いた時だけ**。`cat >` 等の
   シェル経由で書くと刻まれない（2026-09-01 に実測）。
5. **ファイルを新規作成したら同じターンで `MEMORY.md` に1行足す**。
   索引に無いファイルは起動時に存在すら見えない
   （`cache_breakeven_cost.md` が2026-06-02 の作成から約3か月間、索引漏れで不可視だった）。
