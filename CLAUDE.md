...
- **建玉なし**。SOXL 25株（`trades` id=43）は 2026-09-08 に $123.65 で売却済み（手数料込み +$73.94 / +2.46%、円建て -¥3,860）。出口設計の issue #31 は前提の建玉が無くなった。
- **ライブ注文: SOXL 買い 50株・指値 $91.00・GTC（OrderId 5442384385）、保護脚なし、`trades` id=44（placed）**。口座 ¥729,968（約 $4,715）の約96.5%で ADR-028 の cap 超。数量と損切りの扱いは #41。
- **Saxo token は 2026-09-14 21:26 JST に再認証済み**。keepalive はセッション終了で止まるので、次セッションでは失効している可能性がある（蓋閉じ sleep は抑止できない、#22）。
- **Tiingo が約100 req/時以下で HTTP 429**（2026-09-14 23:44 JST）。`docs/api/tiingo/rate-limits.md` の Power プラン記載と矛盾、未確認（#41）。
- **価格 parquet の品質問題（#38、未修正）**: SOXX 日足 OHLC が 2024-03-07 分割で未調整、5分足に米国休場日の合成バー。集計前に除外する。
- **市況（2026-09-14）**: AI 減速提言（events id 462）で SOXX -5.5%（2026-06-22 高値から約 -24%）、SOXL 終値付近 $101.75。Brent $108〜109、FOMC 利上げ確率 約86%。regime は risk_on 0.79 だが半導体急落を反映しない（K-072）。
- **未検証の知見候補（knowledge 未登録）**: 類似20事例で、押し目指値の損切り -18%・利確 +8% は期待値マイナス、損切り -25%・利確 +25% はプラス（#41 コメント、in-sample・時期偏りあり）。
- **カタリスト**: 9/15 21:30 JST NY連銀製造業 → 9/16 21:30 小売売上 → **9/17 03:00 FOMC** → 9/18 日銀（1.25% 利上げ観測）→ 10/1 早朝 Micron 決算。
- **参照**: K-047/K-064/K-065/K-072/K-074/K-080/K-082、ADR-027/ADR-028。開いている作業は issue #22/#31/#37/#38/#39/#40/#41。
