...
- **建玉あり: SOXL 25株 @ $119.879（`trades` id=43 / OrderId 5435911635）。取得原価 $3,004.84**。8/24 引け $111.16 で **-$225.84 / 取得原価比 -7.52%**（エントリー価格比 -7.27%）。**保護逆指値は入っていない**（8/24 に `get_open_orders` が空であることを実測で確認。前セッションの「未確認」は解消し、答えは「無い」）。日中安値 $106.00 でエントリー比 **MAE -11.58%** まで沈み、$106 より上のどの逆指値も刈られていた。無保護が結果的に有利だったが、これは結果論なので学習にしない（K-041）。**次セッションの最初の作業は、無保護のまま NVIDIA 決算に入るかの決定**。
- **Saxo は再認証済み・`/sync-saxo` は3層一致で差分なし**。keepalive はセッション子プロセスなので終了済み、次セッションで再起動が要る。**円建て含み損益は Saxo 表示を信用しない**（同一建玉で50分間に -¥46,445 → -¥32,861 と振れ、評価額の増分 ¥2,904 と整合しない。issue #30）。報告は `(現値 − open_price) × qty` の USD ベースで出す。
- **8/18 以降の半導体安の driver はサムスンの株主還元失望からのメモリ read-across**。制裁も金利も違う。**制裁（Operation Economic Outcast、8/24 14:00 ET）は売りに後行**しており（単独安は 8/21 引けに開始、8/24 プレ 09:14 ET で既に SOXX -1.87%、会見は 14:00 ET）、しかも産油国への「史上最大の金融攻撃」の当日に Brent は -2.5% で引けた。**金利は 8/18・8/19・8/24 と3回連続で反証**（8/24 は10年債 -3bp・30年債 -2bp・BREAKEVEN -2bp で実質金利も低下＝順風の符号なのに SOXX -2.67%）。K-074/ADR-039 の手順が3回とも報道の帰属を却下した。
- **「N日連続で単独安」を異常性の根拠に使わない（[[K-076]]）**。SOXX が4指数中最弱になる日は無条件で 35.2%（n=1300）で、2日連続は5年に111回ある。**異常性は SOXX-SPY 累積スプレッドのパーセンタイルで測る** — 8/24 時点の直近5営業日累積は **-8.57pt で下位 1.1 パーセンタイル**（5年で最も極端な部類）。ただし単調な下げではなく、8/17（+2.05pt、上位90%）と 8/20（+1.36pt、上位82%）に半導体が最強になる往復を挟んでいる。反発期待の見積りにこの差が効く。
- **レジームは 8/25 付で記録済み（risk_on, score 1.21）。VIX_TERM が steep_contango → contango へ後退**（8/21 の 0.818 → 8/24 の 0.854、いずれも同一日確定値で確認済み）。**regime の入力に場中の未確定値が焼き込まれる欠陥が再発**しており（8/21 レコードの VIX 15.73 は確定値 15.13 と乖離）、issue #14 の項目3に実測付きで追記した。
- **未処理**: 8/22 05:00 JST 登録の「米30年債 5.273%」を impact=negative としたが、根拠の「金利が半導体を潰した」帰属が上記で3度崩れた。**事後修正は [[K-056]] の危険があるため `/review-events` で処理する**。あわせて 8/24 22:00 JST 登録の TGA 報道は、当日の10年債 -3bp 超の引き金だったことが事後に判明しており、チャネル3帰属の採点対象。
- **要確認**: ジャクソンホールの論点が「利下げ」でなく**「利上げ」**に寄っている可能性（2026年12月の利上げ確率 67.6% 等）。Tier 3 ソースのみで CME FedWatch の一次確認が取れていない。
- **カタリスト**: **8/27 05:20 JST NVIDIA 決算**（今週最大。8/24 に NVDA 自身が -2% 台まで売られ決算前整理の可能性）→ 8/28 05:45 JST Marvell 決算 → 8/28 23:00 JST Warsh 議長ジャクソンホール基調講演 → 9/8 カナダ報復関税発効 → 9/16-17 FOMC。
- **参照**: K-039/K-041/K-047/K-055/K-056/K-063/K-064/K-068/K-070/K-071/K-073/K-074/K-075/K-076。
