...
- **継続するかは未決着**。[[K-068]](半導体の対 QQQ 劣後は持続せず平均回帰、2日連続の翌日も下回る確率 42.0%/n=283 < 無条件 48.4%)と [[K-039]](プレ下落日の当日下落は 39-47%)は非継続側だが、**[[K-047]] は「金利軸が併走すると反発パターンが減衰する」**とし、今まさに金利軸が生きている。[[K-064]](n=466)より **KOSPI の値動きは SOXL の寄り後と無相関**で、今日のアジア安から当日を推すことはできない。判断は 22:30 JST の寄り後、実出来高のついた価格が出てから。
- **金利は「上昇中」でなく「高値で横ばい」**: 30年債は 2026-02-27 の 4.63% が底、7月に 4.99→5.28 と一気に上げ、8月は 8/7 5.211 → 8/14 5.265 → 8/21 5.276 とほぼ動いていない。8/17 の 5.31% が2026年最高(2007年6月以来)。**再検討条件として置いた「5.2% 割れ」まで 0.08**。
- **issue#26 の前半を完了**: US30Y を **FRED DGS30 単独**で収集開始(5年1246日 backfill 済、`source` 列は全行 `fred`)。当初 yfinance(^TYX)優先で実装したが、**米財務省の日次利回り曲線と照合したら FRED 159/159日 完全一致に対し ^TYX は 0/159・最大差 0.0570** で差し替えた(**K-073**: 複数ソースの相互一致は正確さの証明にならない、一次公表元と照合する)。**判定への組み込み(閾値設計・ADR 起票)は未着手で issue#26 に残る**。
- **参照**: K-041/K-047/K-055/K-063/K-064/K-068/K-070/K-071/K-073。**今週のカタリストは 8/27 05:20 JST NVIDIA 決算 → 8/28 05:45 JST Marvell 決算 → 8/28 23:00 JST Warsh 議長のジャクソンホール基調講演(就任後初)**。9/16-17 FOMC は据置予想が約65%。財務省の長期債バイバック倍増は 9/9 発効〜11/4。
