...
- **Saxo token は失効中（2026-09-05 00:22 JST に refresh token が失効）**。次セッション開始時に `scripts/saxo_oauth_init.py` でブラウザ再認証が必要。`/sync-saxo` はそれまで通らない。
- **keepalive の Deep Idle 停止は塞いだ（issue #22 クローズ、commit 6205337）**: `caffeinate -i -w <pid>` を keepalive の生存中だけ抱かせる実装を入れ、`pmset -g assertions` で `PreventUserIdleSystemSleep` の保持を実機確認した。**ただし蓋を閉じた場合の sleep は抑止できず、依然として失効する**（2026-09-05 00:22 の失効がまさにこれに該当する可能性がある）。**失効時にユーザーへ通知する経路は依然として無い**。
- **8月分の入金 ¥200,000 は 2026-09-01 の sync で台帳に着弾し、現金は完全一致（ライブ settled_cash ¥255,850 == 台帳累計 ¥255,850）**。booking 遅延は実測 ~27時間。**issue #35（3層照合に現金の層が無い）は同日クローズ済み**: 第4層 `SenseiDB.reconcile_cash()` を実装し `sync_saxo.py` から呼んでいる。「✓ 差分なし」は現金を含む4層の一致を意味し、現金がズレている間はそう表示しない。
