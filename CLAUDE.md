...
- **建玉あり: SOXL 25株（`trades` id=43 / OrderId 5435911635）。取得原価 $3,004.84 / 建値 $120.194**。2026-08-29 11:58 JST で $111.30、含み **-$222（建値比 -7.40% / 口座比 -6.8%）**。**保護逆指値なし**。出口設計は issue #31（4択は未決のまま2セッション経過）。
- **セッション再開時の最初の3手**: ①Saxo 再認証（token 失効中。`python scripts/saxo_oauth_init.py`。**callback は localhost なのでこの PC のブラウザでしか完了できない**）②`/sync-saxo` で照合 ③2026-08-28 までの確定足で無効化ラインを引き直す（前回読んだ $112.00 は既に割れた。次のスイング安値は 8/24 の $106.00）。
- **4択を出し直す時は「いつまでに決めないと、どの選択肢が既定で選ばれたことになるか」を先に置く**。2026-08-27 はこれを明示せず、決定が出ないまま期限を通過させた（issue #32）。
- **執行経路を「対処済み」と報告しない（K-077）**。keepalive は3回続けて既知のカタリスト時刻をまたげずに失効した（8/26 79分 / 8/27未明 / 8/27夜 74分）。**3回目の原因は未特定**（マシンスリープ説と DuckDB ロック競合説、いずれも未検証。issue #22）。margin 1500s の有効性も未確定。カタリストをまたぐ経路は**使う直前に実際に叩いて確認する**。
- **`saxo_oauth_init.py` は待機中ずっと DB の排他ロックを保持する（issue #34、未修正）**。`--wait-min` を長く取ると他の全プロセスの DB アクセスを止める。長時間の待機が要る時はこの副作用を承知で使い、終わったら必ず落とす。
- **K-070 は「時計窓」でなく「出来高」の知見に変わった**（8/27 に追試、比 2.70-3.40 で符号反転ゼロ）。`is_thin` フラグでは薄い/厚いを区別できない。`watch_position.py` の静音区間 09:00-21:30 JST は既定値として維持。
- **regime のスナップショットは全6列で観測日が保存されていない**（issue #14）。8/26 分は観測日を `reasoning` に文字列で埋めて手動記録した。8/25 以前は書き換えていない。
- **カタリスト**: 8/28 Marvell 決算・Warsh ジャクソンホール基調講演は通過済み（結果未検証、`/review-events` 対象）→ 9/8 カナダ報復関税発効（電子機器を含む）→ 9/17 03:00 JST FOMC。
- **参照**: K-041/K-055/K-059/K-060/K-063/K-064/K-065/K-070/K-074/K-076/**K-077**。
