...
- **7/11-13 Iran-US戦争再エスカレーション(6月MOU決裂)＋7/13半導体売りは多因子**: 米3夜連続空爆＋Trump港湾封鎖再導入でBrent$85.84(6/15来高値)・VIX17.36へ。ただし**7/13の半導体売り(Nasdaq-1.55%)の主因はIran risk-offでなくAIバリュエーション/循環取引(Channel1/5)＋油→金利(Channel3)**。証拠=売りはIran再燃前の7/6-8に開始(Intel-21%)・VXN-VIX約25年ぶり乖離(テック固有)・Dow耐性はエネルギー株の交絡。当初のIran単一帰属は誤り→**K-052で恒久補正**。イベント4件登録(Iran再燃/circular-financing/油→金利/HBM)。
- **AI強気vs懐疑ディベート(立場交換)の結論**: 対立軸崩壊。**need(2-3年需要は硬い・物理プロキシで独立検証)≠return(ROI未実証)≠price(レバ近期株価)**。強気優勢55%だが左テール太く日付付きカタリスト無し。**SOXLは多年度thesisのbuy-and-hold器でない(地平ミスマッチ)=確信でなくstop統治**(K-053/054・[[artifact dossier]])。
- **ポジション(7/13引け$165.37)**: SOXL 13株=10@$196(**-15.6%**)＋3@$165.40(±0%)。net含み**約-49,672 JPY(-$306)**(7/14プレは$173froth)。**最大の未是正リスク: stop$100=口座36%risk=機能せず**。次の米寄り(22:30 JST)で$156-158構造stop化 or 7株トリム(K-042/K-023: higher low下へ・当日BE引上げ禁止)。**token失効中(keepalive 7/14 23:48終了)→live読取り前に再認証要**。
- **regime**: 前回risk_onのまま**未再判定**(Brent急騰・VIX上昇・contango縮小0.935を反映した`/update-regime`が保留)。データは日足7/13・マクロ7/14。
- **参照**: knowledge(K-052 VXN-VIX帰属/K-053 売り切れ≠株価保護/K-054 need≠return≠price、既存K-047/K-050)・候補チャネル「AI融資・信用構造」(event-channels.md)・トラッカー(#11 KPI/#14 regime.py=VXN-VIX軸/#15 多因子tag ADR)。SOXS禁止・$100祈り握り非推奨は既確定。次決着=**7/16 TSMC Q2決算**(セクターneed再検証)・7/14週 JPM等Q2(ローテーション)。
