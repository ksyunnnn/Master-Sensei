...
1セッションのやりとり・検討・作業を1つの Issue に残す。前例: #41。

- ラベル: `実施記録`
- タイトル: `YYYY-MM-DD[〜DD]: <主題>`
- 本文: サマリ / 最新状況（絶対時刻つき）/ 開いている事項 / 参照
- コメント: 1件1トピック、見出しに絶対日時。時系列に追加し、書き換えない
- 書き方は `docs/record-writing-checklist.md` に従う
- Project #2 に kind=次の一手 で載せ、開いている事項が片付いたら close する
