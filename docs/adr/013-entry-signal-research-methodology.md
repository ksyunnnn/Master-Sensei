...
## Lessons Learned（2026-04-07 Cycle 1完了時追記）

### 設計上の欠陥

1. **反証テストのfloat信号非互換**: random_data_controlとreverse_direction_testがfloat信号で機能しない。設計時にダミーデータでのend-to-endテストを行っていれば防げた。**次Cycleでは全フィルタの信号型別動作検証を設計段階で実施する**

2. **shuffle p値とBH補正の非互換**: 離散p値（1/(n+1)）がBH rank閾値に届かない。**次CycleではBH補正にscreen_signal p値を使う設計を初期段階で固定する**

3. **停止ルールの事前未定義**: 「いつ探索を止めるか」を決めずに開始し、毎回「十分」と宣言しては変更した。**次Cycleでは探索の停止条件を事前に定義する**（例: 全ROUND2_CANDIDATESの実行完了、または累積N時間経過）

4. **Walk-forward 2分割の不十分さ**: 1通りの検証しかできず、分割点依存性を排除できない。**次CycleではCSCV 8ブロック以上を標準とする**

### プロセス上の問題

5. **摩擦後期待値の検証タイミング**: Stage 3の最後に初めて計算し、R2-TD-01に4.5時間費やした後に「摩擦で消える」と判明。**次Cycleでは Stage 1通過直後に摩擦後期待値を概算し、明らかに不足するものを早期脱落させる**

6. **マクロデータ期間の過小評価**: 1年分で研究を開始し、Stage 2前に5年拡張が必要と判明。**次Cycleではデータ準備フェーズで全期間を揃えてから検証を開始する**

7. **事後変更4件**: random_data除外、BH p値ソース変更、マクロ拡張、目標数変更。全て正当な理由があったが、Pre-registration遵守度を2/5に下げた。**次Cycleでは設計固定後の変更はADR/RDRに記録してから実施する**

### 結果の解釈

8. **統計的有意 ≠ 経済的有意**: R2-TD-01（r=0.03、15銘柄）は圧倒的に統計有意だが摩擦後マイナス。大サンプルのSpearman検定は微小な効果も検出する。**次Cycleでは効果量の実用的閾値を事前定義する**

9. **VIXYのコンタンゴ効果**: H-12-03-Sの61.5%のうち58.9%が構造的減価。**VIXY信号を評価する際はベースラインショート勝率との差分（追加エッジ）で判断する**
