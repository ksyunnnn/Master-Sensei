...
1. **random_data_control問題の解決** — 全bool信号194件がStage 1と矛盾する閾値で全滅。Stage 1判定から除外して再評価が妥当
2. **Stage 2検証** — 通過14仮説にBH法・Walk-forward・レジーム安定性・複数銘柄再現を適用
3. **Round 2（探索ラウンド）** — 通過シグナルの深掘り+新仮説生成
