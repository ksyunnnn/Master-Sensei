...
- sensei.duckdb: レジーム2件、予測2件（解決1/未解決1、Brier 0.2025）、知見14件（全件検証済み）、イベント44件（79%レビュー済み）、skill_executions 3件
...
3. **`/review-events`（残り9件）** — 3/26以降の直近イベントが3日経過後に対象
...
- /review-events残り9件（3/26以降が3日経過後に対象）
