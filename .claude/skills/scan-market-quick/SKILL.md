...
検索結果を、**チャネル台帳（`docs/event-channels.md`）への照合**で3つに分類する（判定前に台帳を一読, ADR-034）:

- **登録**: 台帳のいずれかのチャネルに明確にヒットし、ソースが Tier 1-2 → イベント登録する（該当チャネルを impact_reasoning に明記）
- **深掘りフラグ**: 伝播が疑わしいが不確実、または新規チャネル候補 → 登録せず報告（quick では取りこぼしをフラグで surface する＝サイレント損失にしない。通常版の「迷えば neutral 捕捉」に相当する quick の扱い）
- **スキップ**: どのチャネルにも当たらない／既存と重複 → 何もしない

inclusion に「実際に動いたか（realized impact）」は使わない（look-ahead 禁止）。動いたかの検証は `/review-events` の事後採点で行う。
