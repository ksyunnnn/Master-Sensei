...
**該当時の設計**:
- **部分利確は「明記」でなく resting order として置く**（手順5の発注時点で、本体 TP と別に部分利確指値を実際に発注する）。MFE 接近で**自動約定**させ、watching/手動介入に依存させない。水準だけ決めて「当日その値で手動執行」は不十分——監視できない時間帯に水準を通過すると取り逃す（後出しの判断変更・BE引上げ禁止、K-023 と整合）。
  - **K-044 実証 #17（2026-06-12）**: 単一 TP$243（壁$231-234 の上）は MFE **$237.90** に届かず**約定しなかった**。利益確定は引け前 15:10 ET の**手動売り**に依存した＝起きていなければ週末の二項クラスタ（イラン署名＋FOMC）へ +9.6% を晒したまま持ち越していた。教訓: 壁の**手前**（直近イントラデイ高値の下、例 $237-238）に部分利確指値を resting で置けば、watching なしで MFE を確定できた。
- **単一 OCO 指値だけに依存しない**。天井 TP は「ドンピシャの天井」を要求し空振りリスクが高い（#17 で実証: $243 は未約定）。部分利確の水準は σ 単独でなく**直近イントラデイ高値（5分足の抵抗）**から割り出す。
...
| 部分利確 | {跨ぎ時: ${水準} で {M}/{N}株} / {跨がない時: なし(理由)} | イベント跨ぎ判定 (手順3.7, K-044)。直近イントラデイ高値の手前に **resting order で発注**（手動介入に依存しない、#17実証） |
