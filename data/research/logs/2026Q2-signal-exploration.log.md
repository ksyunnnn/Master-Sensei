...
**最終結果**: 実弾確定 **H-18-03のみ**（3日連続下落→ロング、TQQQ/TECL、摩擦後+1.4%/回）。R2-TD-01は摩擦後マイナスで不採用。H-12-03-Sはコンタンゴ+2.5%のみで保留。
...
| e2 | Round 2（探索ラウンド、30仮説×348実行） | **完了** | e1 |
| e3 | Stage 3（実用性評価、摩擦後期待値検証） | **完了** | e2 |
| e4 | 最終評価レポート + バイアス監査 | **完了** | e3 |
| f | Confirmatory Round（H-18-03固定プロトコル再検証+CSCV） | 未着手 | e4 |
| g | H-18-03実弾化実装（Skill/ツール） | 未着手 | f |
...
### 2026-04-05〜07 session 12-14: Stage 2 + Round 2 + Stage 3 + 最終評価 + 実弾化基盤

**やったこと:**
1. random_data_control問題解決（Stage 1判定から除外、通過27→39→48）
2. FRED 9系列を5年拡張、マクロ仮説再実行（通過48）
3. Stage 2実装・実行（4フィルタ、4仮説通過）
4. Round 2探索（30仮説×348実行、R2-TD-01が15銘柄で通過）
5. Stage 3実用性評価 + 摩擦後期待値検証（H-18-03のみ実弾成立）
6. バイアス監査フレームワーク導入・適用（Premortem + Kahneman 12問）
7. Confirmatory Round（CSCV 100%、パラメータ2-4日BH通過）
8. /signal-checkスキル + src/signals/基盤実装（ECA Condition層）
9. ドキュメント整備（Diátaxis: Tutorial/Reference/How-to）
10. 最終評価レポート（Harvey閾値/研究者自由度/バイアス監査）

**重要な発見:**
- shuffle p値の離散制限（1/(n+1)）がBH補正と原理的に非互換
- R2-TD-01は15銘柄で統計有意だが摩擦後-0.07%で経済的に無意味
- H-12-03-Sの61.5%のうち58.9%はVIXYコンタンゴ（追加エッジ+2.5%のみ）
- 研究者の自由度4件（事後変更）により全結果はexploratory扱い→Confirmatory Roundで修正

**失敗:**
- 「7本で十分」→バイアス監査⚠️9で保留判定。探索打ち切りバイアス
- 「半年後にデータが増える」→5年→5.5年の10%増は無意味。怠慢な議論
- 摩擦後検証をStage 2前にすべきだった（R2-TD-01に4時間費やした後に発覚）

**最終結果:**
- 実弾確定: **H-18-03（3日連続下落→ロング）のみ**
- TQQQ: 勝率64.8%, 摩擦後+1.42%/回, 年+30.6%
- CSCV 70/70 OOS正リターン、パラメータ2-4日全てBH通過
...
- [x] **e4: 最終評価レポート + バイアス監査**（Harvey閾値/研究者自由度/Kahneman 12問、Pre-reg遵守2/5）
- [x] **e3: Stage 3 + 摩擦後検証**（H-18-03のみ実弾成立、R2-TD-01摩擦後マイナス、H-12-03-Sコンタンゴ+2.5%のみ）
- [x] **e2: Round 2探索**（30仮説×348実行、R2-TD-01が15銘柄で通過→摩擦後不成立）
