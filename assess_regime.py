...
入力値スナップショットをregime_assessmentsに埋め込む（ADR-009）。
...
def load_latest_macro(cache: CacheManager) -> dict:
    """各マクロ指標の最新値をParquetから取得（ADR-009: Parquetに一本化）"""
    values = {}
    for series in SERIES_NAMES:
        df = cache.load_macro(series)
        if not df.empty:
            values[series] = float(df["value"].iloc[-1])
    return values
...
def run(dry_run: bool = False):
    ...
    db.save_regime(
        ...
        vix_value=latest.get("VIX"),
        vix3m_value=latest.get("VIX3M"),
        hy_spread_value=latest.get("HY_SPREAD"),
        yield_curve_value=latest.get("YIELD_CURVE"),
        oil_value=latest.get("BRENT"),
        usd_value=latest.get("USD_INDEX"),
    )
